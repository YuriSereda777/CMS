...
const Users = () => {
  ...
  useEffect(() => {
    getUsers(
      { url: "http://localhost:80/cms-api/users.php" },
      (data) => {
        setUsers(data.map(element => ({...element, id: parseInt(element.id)})));
  ...
  return (
    <Table
      title='Users' table={table} elements={users} elementsPerPage={10} pagination={true}
      isLoading={isLoading} error={error}
      search={true} searchInputValue={inputValue} searchHandler={searchHandler} filteredArray={filteredArray}
