...
const Categories = () => {
  ...
  useEffect(() => {
    getCategories(
      ...
      (data) => {
        setCategories(data.map(element => ({id: parseInt(element.id), name: element.name, number: parseInt(element.number)})));
      }
    );
  }, [getCategories]);
  ...
  return (
    <Table
      title='Categories' table={table} elements={categories} pagination={false}
      isLoading={isLoading} error={error}
      search={false} filteredArray={categories}
